...
# --- Header ---
st.title("🌊 Crisis Replay — Historical Stress Test")
st.markdown("""
This is the **historical** stress test: it takes the **actual day-by-day path** of a past crisis and applies it to your current weights — so you see what *really happened* to a portfolio like yours (real drawdown, real recovery time), benchmarked against the S&P 500 (SPY).

> 🧭 **Two kinds of stress test — don't mix them up:**
> **This page (Crisis Replay)** = the *actual past path*. What really happened.
> **Monte Carlo Simulator → Crisis regime** = a *simulated future range* using a crisis period's volatility. What *could* happen if such conditions return.
""")
st.divider()
